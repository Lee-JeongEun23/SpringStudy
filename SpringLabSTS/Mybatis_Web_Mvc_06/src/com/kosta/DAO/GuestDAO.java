...
public class GuestDAO {
	...
	public int insert(GuestDTO guestobj){
		...
		try{
			//factory.openSession(true); autocommit //이러면 오토커밋
			//factory.openSession(); app (commit ,rollback)
					
			session = factory.openSession();//작업을 method 단위 //이렇게 비워놓으면 커밋과 롤백 직접 설정
			
			int result = session.insert("GUEST.insertGuest",guestobj);
			session.commit();
			return result;
		}catch(Exception e){
			session.rollback(); //exception에서 롤백
			return 0;
		}finally{
	...
	public int delete(int num){
		...
		try{
			...
			session = factory.openSession(true);//작업을 method 단위 //autocommit
