...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#sendBtn').click(function() {
	       ...
	       $(window).on("beforeunload", function(){ //해당 이벤트는 윈도우, 문서 및 리소스가 언로드 될 때 발생
				disconnect();
		    });
	})
	
	function connect() { //입장 버튼 클릭시 작동 함수(웹소켓 생성)
		wsocket = new WebSocket("ws://192.168.6.19:8090/WebSocket_4Team/Chat-ws.do?cmd=join&room=${room}");
	...
	function appendMessage(data) {
		if (data.type == "my") {
			...
			setChattingMember(data.owner, data.users); //채팅 인원 목록 나타내는 함수
		} else {
	...
	function setChattingMember(owner, members){ //채팅 인원 목록 나타내는 함수
		console.log("mem");
		console.log(owner);
		console.log(members);
		$("#memberArea").empty();
